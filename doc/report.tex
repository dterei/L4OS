...
\item A \emph{swap out request} is pushed to the head of the request queue, then the request started.  To put the request at the end of the queue would be incorrect, as it is the page faults which are queued.  A swap out request is just part of dealing with the page fault.
\item A slot in the swap file is allocated in a process similar to frame allocation.  Swapped pages must be kept track of in order to free all slots when a process finishes.
\item The swap out request will open the relevant swap file given in the region list (although currently always ``.swap'').  This begins a chain of continuations which open, lseek, write, then close the file in sequential order.  Writing will actually involve several tries as the NFS buffer is not large enough for an entire page.  Currently, 
